...
class GenerateRandomQuesionIdx extends React.Component {
  ...
  componentDidMount() {
    fetch(`http://localhost:4000/totalitems`)
      ...
      this.updateTimer = setInterval(() => this.updateQuestion(), NEXT_QUESTION_INTERVAL*1000);
  }
  ...
  render() {
    ...
    if (!DataIsLoaded) {
      ...
      return (
        <Question qIdx={this.randomQuestionIdx} updateInterVal={NEXT_QUESTION_INTERVAL} />
      );
    }
  }
}
